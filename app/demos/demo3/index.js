...
console.log(SoundToColor);
const COLORS = [new Color(SoundToColor[0].r/255, SoundToColor[0].g/255, SoundToColor[0].b/255)];//["#ffffff"].map((col) => new Color(col));
const STATIC_PROPS = {
  nbrOfPoints: 4,
  speed: 0.03,
...
class CustomLineGenerator extends LineGenerator {
  ...
  //Add new line with custom parameters
  addCustomLine(params){
    const line = super.addLine({
      width: 2/params.intensity,
      length: 50/params.intensity,
      visibleLength: getRandomFloat(0.5, 0.8),
      position: new Vector3(
        POSITION_X,
        0.3,
        getRandomFloat(-1, 1),
